...
export default function BuildingPoints({ setHoveredId, hoveredId }: BuildingPointsProps) {
    ...
    return (<>
        ...
        {selectedId && (<div className="absolute top-1/4 left-[40%] bg-black/40  text-white p-8 rounded shadow-lg ">
            <p className="absolute bg-white rounded-full text-black right-1 top-1 cursor-pointer" onClick={() => setSelectedId(null)}><IoMdCloseCircle /></p>
            <p>{Towers.find((t) => t.id === selectedId)?.popuptext1}</p>
            <p>{Towers.find((t) => t.id === selectedId)?.popuptext2}</p>
        </div>)}
    </>)
}